...
export function UMKMSection() {
  ...
  const translations = {
    id: {
      title: 'UMKM Lokal Pilihan',
      subtitle: 'Dukung produk lokal Jogja yang keren dan berkualitas!',
      categories: [
        { id: 'all', label: 'Semua' },
        { id: 'culinary', label: 'Kuliner' },
        { id: 'craft', label: 'Kerajinan' },
        { id: 'fashion', label: 'Fashion' },
        { id: 'creative', label: 'Kreatif' }
      ],
      startingFrom: 'Mulai dari',
      viewOnMap: 'Lihat di Map'
    },
    en: {
      title: 'Featured Local SMEs',
      subtitle: 'Support cool and quality local Jogja products!',
      categories: [
        { id: 'all', label: 'All' },
  ...
  const umkm: Record<string, UMKMItem[]> = {
    id: [
      {
        id: 1,
        name: 'Gudeg Yu Djum',
  ...
  const currentUMKM = umkm[locale] || umkm.id;
  const filteredUMKM = activeCategory === 'all'
    ? currentUMKM
    : currentUMKM.filter(item => item.category === activeCategory);
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-100 to-amber-100 dark:from-emerald-900/30 dark:to-amber-900/30 rounded-full mb-4">
            ...
            <span className="text-sm text-emerald-700 dark:text-emerald-300 font-medium">Support Local Business</span>
          </div>
